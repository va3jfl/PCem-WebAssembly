...
typedef struct codeblock_t {
        ...
        /* PCem-web: compiled blocks are runtime-generated wasm modules; this
           is the shared-table index of the block function (0 = none). */
        uint32_t wasm_fn_idx;
        /* Block contains non-recompiled instructions (arbitrary handlers,
           possibly port I/O) — not replayable under the differential oracle. */
        uint8_t oracle_taint;
        /* Visits since creation (or since last dirty-invalidation). A wasm
           WebAssembly.Module compile costs orders of magnitude more than a
           native block emit, so a block must prove itself hot
           (wjit_compile_threshold visits) before the recompiler pays for it;
           cold visits run on the interpreter. Reset on invalidation so
           self-modifying pages must re-prove hotness instead of paying a
           compile per modification. */
        uint32_t wasm_hits;
#endif
} codeblock_t;
...
#ifdef __EMSCRIPTEN__
/* PCem-web: compiling this block measured as not worth it (dominated by
   call-outs to interpreter handlers — I/O, HLT, segment ops). It runs on the
   interpreter permanently; the recompiler never retries. This is what keeps
   the wasm JIT strictly not-slower than the interpreter on dispatch/I-O-bound
   code while keeping its win on compute. */
#define CODEBLOCK_WASM_NOJIT 0x100
/* PCem-web: block contains recompiled port I/O. Field-measured (Win98 boot,
   startup-sound chop at ~80% guest speed): making I/O blocks compile-eligible
   at the normal 16-visit threshold triggers a boot-time compile storm —
   thousands of driver-init/probe blocks each paying a synchronous module
   compile. I/O blocks therefore need WJIT_IO_THRESHOLD visits: transient
   boot code never compiles, genuinely hot I/O loops still do. */
#define CODEBLOCK_WASM_IO 0x200
#endif
