...
/* PCem-web: generated code lives in runtime wasm modules, not in these
   chunks; blocks keep only their single default chunk, so one chunk per
   codeblock slot (+headroom) suffices — and this allocation comes out of
   the wasm heap, so keep it small. */
#define MEM_BLOCK_NR 32768
#else
