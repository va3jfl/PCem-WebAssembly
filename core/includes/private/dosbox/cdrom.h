...
class CDROM_Interface_Image : public CDROM_Interface {
    private:
        ...
        /* PCem-web: lazily-read CD image over a local browser File or a
           hosted URL ("browser:<id>:<name>" paths; see wasm-diskio.c). */
        class BrowserFile : public TrackFile {
            public:
                BrowserFile(const char *filename, bool &error);
                bool read(Bit8u *buffer, int seek, int count);
                int getLength();
