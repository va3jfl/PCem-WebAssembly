...
/* PCem-web: timer callbacks must really be void(*)(void*) — calling a
   void() through a cast pointer is a signature-mismatch trap in wasm. */
static void ide_callback_sec_timer_cb(void *priv) {
        (void)priv;
        ide_callback_sec();
}
...
/* PCem-web: timer callbacks must really be void(*)(void*) — calling a
   void() through a cast pointer is a signature-mismatch trap in wasm. */
static void ide_callback_pri_timer_cb(void *priv) {
        (void)priv;
        ide_callback_pri();
}
...
static void *ide_init() {
        ...
        timer_add(&ide_timer[0], ide_callback_pri_timer_cb, NULL, 0);
        timer_add(&ide_timer[1], ide_callback_sec_timer_cb, NULL, 0);
