...
void loadnvr() {
        ...
        if (!f) {
                ...
                if (!enable_sync) {
                        ...
                        /* PCem-web: a first boot with no NVR file and time sync
                           enabled left the whole CMOS at 0xFF: REGB.SET stuck on
                           (so nvr_onesec never calls rtc_tick) and the time
                           registers unreadable. Real firmware boot blocks
                           (e.g. Intel AMIBIOS on the PB570) poll the RTC seconds
                           register until it looks sane and therefore hang on a
                           black screen. Natively this is masked because savenvr()
                           creates the file at the end of the first session; in
                           the browser every page load starts from a fresh FS, so
                           every boot is a first boot. Initialise to the same end
                           state the file-exists path produces: sane REGA/REGB and
                           time registers synced from the host clock. */
                        nvrram[RTC_REGA] = 6;
                        nvrram[RTC_REGB] = RTC_2412;
                        nvrram[RTC_REGC] = 0;
