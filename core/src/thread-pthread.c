...
#include <stdio.h>
#include <time.h>
#include <unistd.h> /* PCem-web: usleep() prototype (implicit decls are traps in wasm) */
#include <pthread.h>
#include "thread.h"

/* PCem-web: events must be STATEFUL (Win32 manual-reset semantics, which the
   core's blit/CD/Voodoo handshakes were written against — see win-thread.c and
   the identical fix upstream already carries in qt-thread.c). The stateless
   version loses a thread_set_event() that fires while the consumer is between
   waits: the video blit thread then sleeps forever with blit_data.busy stuck,
   and the emulation thread wedges inside video_wait_for_blit(). */
