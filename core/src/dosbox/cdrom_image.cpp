...
/* PCem-web: see wasm/platform/wasm-diskio.c */
extern "C" int wasm_diskio_parse(const char *fn);
extern "C" double wasm_diskio_size(int id);
extern "C" int wasm_diskio_read(int id, uint64_t byte_offset, int len, void *buf);

CDROM_Interface_Image::BrowserFile::BrowserFile(const char *filename, bool &error) {
	id = wasm_diskio_parse(filename);
	error = (id < 0);
}

bool CDROM_Interface_Image::BrowserFile::read(Bit8u *buffer, int seek, int count) {
	if (seek < 0 || count <= 0)
		return false;
	return wasm_diskio_read(id, (uint64_t)(unsigned)seek, count, buffer) == 0;
}

int CDROM_Interface_Image::BrowserFile::getLength() {
	return (int)wasm_diskio_size(id);
}
#endif
...
bool CDROM_Interface_Image::LoadIsoFile(char *filename) {
	...
	bool error;
#ifdef __EMSCRIPTEN__
	if (!strncmp(filename, "browser:", 8))
		track.file = new BrowserFile(filename, error);
	else
#endif
