...
static void invalidate_block(codeblock_t *block) {
        ...
        codegen_wasm_free_block(block); /* PCem-web: release the wasm table slot */
#endif
}
...
static void delete_dirty_block(codeblock_t *block) {
        ...
        codeblock_tree_delete(block);
#ifdef __EMSCRIPTEN__
        /* PCem-web fix: dirty-list blocks can still own a wasm table slot
           (normally recycled by the recompile epilogue, which never runs for
           a block deleted here) — without this the slot and its compiled
           module leaked for the session. */
        codegen_wasm_free_block(block);
#endif
        block_free_list_add(block);
}
...
#ifdef __EMSCRIPTEN__
        block->wasm_hits = 0; /* must prove hotness before a wasm compile (see codegen.h) */
#endif
