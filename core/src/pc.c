...
void initpc(int argc, char *argv[]) {
...
#endif
        {
                if (cdrom_drive == CDROM_IMAGE) {
                        int image_ok = 0;
                        if (!strncmp(image_path, "browser:", 8)) {
                                /* PCem-web: lazily-read browser/hosted image — no
                                   file to stat; image_open() validates it. */
                                image_ok = !image_open(image_path);
                        } else {
                                FILE *ff = fopen(image_path, "rb");
                                if (ff) {
                                        fclose(ff);
                                        /* PCem-web: image_open() failure (unreadable/invalid
                                           image) must not be ignored — it returns before
                                           binding the `atapi` ops, and the next IDE reset
                                           calls atapi->stop() through a stale/NULL pointer. */
                                        image_ok = !image_open(image_path);
                                }
                        }
                        if (!image_ok) {
#ifdef __EMSCRIPTEN__
                                /* PCem-web: never fail silently — the machine
                                   still boots, but with an empty CD drive. */
                                pclog("cdrom: image_open failed for '%s' — booting with an empty drive\n", image_path);
                                MAIN_THREAD_ASYNC_EM_ASM({
                                        if (Module.onPCemMessage)
                                                Module.onPCemMessage("CD image could not be opened",
                                                                     "The CD-ROM image could not be read, so the machine booted with an empty CD drive. Re-attach the ISO under Configure → Media and boot again.",
                                                                     true);
                                });
#endif
#if __unix
