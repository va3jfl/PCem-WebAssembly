...
/* PCem-web: the roms directory lives on the web server (/roms, same layout
   as a desktop install). On a local miss, pull the file over HTTP into the
   virtual FS and retry — the web build "looks at the roms folder" exactly
   like the native one, one file at a time, no manifests. */
extern int wasm_rom_fetch(const char *fn);
#endif
