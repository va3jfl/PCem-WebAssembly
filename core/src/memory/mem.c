...
/* PCem-web: wasm JIT differential-oracle hooks (wasm/jit/wasm-jit-oracle.c).
   With the oracle off (the normal case) these are two predictable branches
   on cold paths and one on the RAM-page store path. */
extern int wjit_oracle_phase;   /* 0 off, 1 JIT run, 2 interpreter replay */
extern int wjit_oracle_nofill;  /* suppress TLB-lookup refill during oracle */
/* handlers defined later in this file and not exposed via mem.h */
...
void wjit_oracle_note_ram_write(uint32_t hostptr, int size, uint32_t oldv, uint32_t newv);
void wjit_oracle_note_io(void);

static inline int wjit_write_map_is_safe(mem_mapping_t *map) {
        return (map->write_b == mem_write_ram || map->write_b == mem_write_remapped || map->write_b == mem_write_null ||
                map->write_w == mem_write_ramw || map->write_w == mem_write_remappedw || map->write_w == mem_write_nullw ||
                map->write_l == mem_write_raml || map->write_l == mem_write_remappedl || map->write_l == mem_write_nulll);
}
static inline int wjit_read_map_is_safe(mem_mapping_t *map) {
        return (map->read_b == mem_read_ram || map->read_b == mem_read_remapped || map->read_b == mem_read_bios ||
                map->read_b == mem_read_romext || map->read_b == rom_read || map->read_w == mem_read_ramw ||
                map->read_w == mem_read_remappedw || map->read_w == mem_read_biosw || map->read_w == mem_read_romextw ||
                map->read_w == rom_readw || map->read_l == mem_read_raml || map->read_l == mem_read_remappedl ||
                map->read_l == mem_read_biosl || map->read_l == mem_read_romextl || map->read_l == rom_readl);
}
#define WJIT_ORACLE_WRITE_DISPATCH(map)                                                                                                  do {                                                                                                                                     if (wjit_oracle_phase && (map) && !wjit_write_map_is_safe(map))                                                                          wjit_oracle_note_io();                                                                                           } while (0)
#define WJIT_ORACLE_READ_DISPATCH(map)                                                                                                   do {                                                                                                                                     if (wjit_oracle_phase && (map) && !wjit_read_map_is_safe(map))                                                                           wjit_oracle_note_io();                                                                                           } while (0)
#else
#define WJIT_ORACLE_WRITE_DISPATCH(map)
#define WJIT_ORACLE_READ_DISPATCH(map)
#endif
...
#ifdef __EMSCRIPTEN__
        if (wjit_oracle_nofill) /* PCem-web: keep lookups cold during oracle runs */
                return;
#endif
        //        return;
...
void writemembl(uint32_t addr, uint8_t val) {
        ...
        WJIT_ORACLE_WRITE_DISPATCH(map);
        if (map && map->write_b)
...
void mem_write_ramb_page(uint32_t addr, uint8_t val, page_t *p) {
        if (val != p->mem[addr & 0xfff] || codegen_in_recompile) {
                ...
                //        mask);
#ifdef __EMSCRIPTEN__
                if (wjit_oracle_phase) /* PCem-web: JIT oracle write journal */
                        wjit_oracle_note_ram_write((uint32_t)(uintptr_t)&p->mem[addr & 0xfff], 1, p->mem[addr & 0xfff], val);
#endif
...
#ifdef __EMSCRIPTEN__
                if (wjit_oracle_phase) /* PCem-web: JIT oracle write journal */
                        wjit_oracle_note_ram_write((uint32_t)(uintptr_t)&p->mem[addr & 0xfff], 2,
...
#ifdef __EMSCRIPTEN__
                if (wjit_oracle_phase) /* PCem-web: JIT oracle write journal */
                        wjit_oracle_note_ram_write((uint32_t)(uintptr_t)&p->mem[addr & 0xfff], 4,
